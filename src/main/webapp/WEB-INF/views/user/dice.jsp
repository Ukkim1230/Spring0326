  ...
  // 주사위 굴리기 함수
  function rollDice() {
    console.log("이전 주사위 값:", diceValues.join(", "));
    ...
    diceElements.forEach((diceElement, index) => {
      if (!diceElement.classList.contains('selected')) {
        ...
        console.log("주사위", index + 1, "새 값:", newValue);
      } else {
        // 선택된 주사위는 현재 값 유지
        diceValues[index] = getDiceValue(diceElement);
        console.log("주사위", index + 1, "유지된 값:", diceValues[index]);
      }
    });
    
    console.log("새로운 주사위 값:", diceValues.join(", "));
  }
  ...
  function setInitialDiceValues() {
    ...
    diceValues.forEach((value, index) => {
      ...
      console.log("주사위", index + 1, "값 설정:", value);
    });
    
    // 가능한 점수 업데이트
    updatePossibleScores();
    console.log("초기 주사위 값:", diceValues.join(", "));
  }

  // 게임 시작 시 초기화
  window.addEventListener('DOMContentLoaded', function() {
    console.log("페이지 로드됨");
    diceValues = [1, 3, 4, 3, 5]; // 초기 주사위 값 설정
    setInitialDiceValues();
  });
